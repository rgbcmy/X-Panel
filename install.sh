...
install_free_version() {
    ...
    install_x-ui() {
        ...
        if [ $# == 0 ]; then
            ...
            echo ""
            sleep 2
            wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/rgbcmy/x-panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
            if [[ $? -ne 0 ]]; then
            ...
            last_version=$1
            url="https://github.com/rgbcmy/x-panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
            echo ""
            echo -e "--------------------------------------------"
        ...
        wget -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/rgbcmy/x-panel/main/x-ui.sh
    ...
    echo -e "----------------------------------------------"
    echo ""
    echo -e "${green}〔X-Panel面板〕项目地址：${yellow}https://github.com/rgbcmy/x-panel${plain}" 
    echo ""
    echo -e "${green} 详细安装教程：${yellow}https://rgbcmy.blogspot.com/2025/09/x-panel.html${plain}"
    echo ""
    echo -e "----------------------------------------------"
    echo ""
    echo -e "-------------->>>>>>>赞 助 推 广 区<<<<<<<<-------------------"
