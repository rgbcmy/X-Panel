...
last_version=$(curl -Ls "https://api.github.com/repos/rgbcmy/x-panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/rgbcmy/x-panel/main/x-ui.sh
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/rgbcmy/x-panel/master/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/rgbcmy/x-panel/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/rgbcmy/x-panel/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
bash <(curl -Ls https://raw.githubusercontent.com/rgbcmy/sublink/main/install.sh)
...
——————————————————————
  ...
  ${yellow}https://github.com/rgbcmy/x-panel${plain}
  ${green}详细〔安装配置〕教程${plain}
  ${yellow}https://rgbcmy.blogspot.com/2025/09/x-panel.html${plain}
——————————————————————
